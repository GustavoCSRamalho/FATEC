...
public class SecretarioFinanceiro {
	private List<Item> entradaCaixa;
	private List<Item> saidaCaixa;
	private List<Item> entradaSaida;
	private final String endereco; 
	private Dados dados;
	
	public SecretarioFinanceiro() {
		entradaCaixa = new ArrayList<Item>();
		saidaCaixa = new ArrayList<Item>();
		dados = new Dados();
	...
	public final void salvaDados() throws IOException {
		...
		entradaSaida = new ArrayList<Item>();
	...
	public final void recuperaDados() throws IOException, ClassNotFoundException {
		...
		List<Item> empresaDados = (List<Item>) lerObjeto.readObject();
		if (empresaDados.size() != 0) {
			System.out.println("\nProdutos recuperados: \n");
			for(Item item: empresaDados) {
